...
<template>
  <div   class="flex flex-col  h-full  w-[480px] ">
    <div class="flex flex-row justify-end">
      <span v-if="isDisabled" class="bg-slate-400 z-70 uppercase px-3 pr-[30px] font-semibold text-gray-50 rounded-t-2xl">Неактивно</span>
      <span class="bg-sky-700 z-80 uppercase px-3 py-0.5 ml-[-20px] text-sm  text-gray-50 rounded-t-2xl">Банкрутство</span>
    </div>
    ...
    <div class="relative h-full text-sm bg-white dark:bg-opacity-10 rounded-2xl rounded-tr-none p-3 shadow-xl">
      <table class="pb" :class="isDisabled ? 'text-gray-400':''">
        ...
        <tr class="border-t-2 border-gray-200 dark:border-slate-500 border-dotted"><th  class="pt-4">Дата події:</th><td class="pt-4">{{ isoDateStrToShortDateStr(g.date)}}</td></tr>
