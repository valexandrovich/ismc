...
<template>
  ...
  <div   class="flex flex-col h-full  w-[400px]">
    <div class="flex flex-row justify-end">
      ...
      <span class="bg-sky-700 z-80 uppercase px-3 py-0.5 ml-[-20px] text-sm text-gray-50 rounded-t-2xl">Недійсний паспорт (МВС)</span>
    </div>
    ...
    <div class="relative h-full text-sm bg-white dark:bg-opacity-10 rounded-2xl rounded-tr-none p-3 shadow-xl">
      <table class="pb" :class="isDisabled ? 'text-gray-400':''">
        <tbody>
        <tr><td colspan="2"  class=" pb-2 text-gray-400 font-semibold  border-b-2 border-gray-200 dark:border-slate-500 border-dotted"><span>Паспорт громадянина України</span></td></tr>
        <tr class="text-lg text-nowrap"><th>Серія та номер:</th><td class="text-nowrap font-semibold">{{g.series}} {{g.number.toString().padStart(6, '0')}}</td></tr>
        <tr><th class="text-nowrap">Орган:</th><td>{{g.ovd}}</td></tr>
        <tr><th>Тип:</th><td>{{g.type}}</td></tr>
        <tr><th>Статус:</th><td>{{g.status}}</td></tr>
        <tr class="border-t-2 border-gray-200 dark:border-slate-500 border-dotted"><th class="pt-4">Дата події:</th><td class="pt-4">{{isoDateStrToShortDateStr(g.theftDate)}}</td></tr>
