...
import {useFilesPpStore} from "@/stores/files.pp.js";
import customAxios from "@/utils/customAxios.js";
import {Icon} from "@iconify/vue";
import {dateToShortDateStr, isoDateTimeStrToShortDateStr} from "@/utils/convertor.js";
import Toastify from "toastify-js";

const filesPpStore = useFilesPpStore();
const isLoading = false


onMounted(()=>{
  filesPpStore.fetchData();
})

const uploadFile = (id) => {
  customAxios.post("/uploader/pp/upload/" + id)
      .then(resp => {
        console.log(resp)
...
const deleteFile = (id) => {
  customAxios.delete("/uploader/pp/delete/" + id)
      .then(resp => {
        Toastify({
          text: 'Файл видалено ',
          duration: 3000,
  ...
  <div v-if="isLoading" class="absolute top-0 left-0 bg-white dark:bg-slate-600 bg-opacity-80 z-40 w-full rounded-lg">
    ...
    <div class="">
      <table class="w-full">
        ...
        <tbody>
        <tr v-for="file in filesPpStore.sortedFiles" :key="file.id">
